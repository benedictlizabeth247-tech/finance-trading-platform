...
export function CreditForm({ userId }: { userId: string }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    setMessage("")
    try {
      const response = await fetch("/api/admin/credits", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ userId, amount, reason }) })
      const result = await response.json()
      if (!response.ok) throw new Error(result.error || "Credit update failed")
      setMessage(`Updated sandbox credits: ${result.wallet.available ?? 0} available`)
      setAmount(""); setReason("")
    } catch (error) { setMessage(error instanceof Error ? error.message : "Credit update failed") }
    finally { setSaving(false) }
  }

  return <form onSubmit={submit} className="mt-5 grid gap-3 rounded-2xl border border-[#29413A] bg-[#101A18] p-4 sm:grid-cols-2"><p className="text-xs font-bold text-[#8DE0BD] sm:col-span-2">Sandbox test credits only · capped at $10,000 · every change is recorded in the audit ledger.</p>
