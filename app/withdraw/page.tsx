...
const money=(n:number,c="NGN")=>new Intl.NumberFormat("en-NG",{style:"currency",currency:c,maximumFractionDigits:2}).format(n)
...
export default function WithdrawPage(){
 ...
 const submit=async()=>{
  ...
  setBusy(true)
  try{
   const result=await createWithdrawalRequest({amount:n,destinationType,destination,currency:wallet?.currency||"NGN", autopilot})
   setSubmitted(true)
   toast({title:"Withdrawal request submitted",description:`Reference ${result.id}`})
 ...
 return <main className="min-h-screen bg-[#EEF2F1] pb-32 text-[#183A36]">
  ...
  <div className="space-y-4 p-4">
   <Card className="rounded-[26px] border-none bg-[#183A36] p-5 text-white"><p className="text-[9px] uppercase tracking-widest text-white/60">Available Funding Balance</p><p className="mt-2 text-[28px] font-black">{money(Number(wallet?.available||0),wallet?.currency||"NGN")}</p></Card>
   {!submitted?<>
