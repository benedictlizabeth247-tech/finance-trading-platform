...
export default function FundAccountPage() {
  ...
  const startStripeCheckout = async () => {
    ...
    if (!value || value <= 0) { toast({ variant: 'destructive', title: 'Enter an amount', description: 'Enter the amount you want to fund.' }); return }
    setSubmitting(true)
    try {
      const response = await fetch('/api/payments/stripe/checkout', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ amount: Math.round(value * 100) }) })
      const payload = await response.json()
      if (!response.ok || !payload.url) throw new Error(payload.error || 'Stripe funding is unavailable.')
      window.location.assign(payload.url)
    } catch (e: any) { toast({ variant: 'destructive', title: 'Stripe funding unavailable', description: e?.message || 'Please try another method.' }) } finally { setSubmitting(false) }
  }

  const startFiatCheckout = async () => {
    const value = Number(amount || 0)
    if (!value || value <= 0) { toast({ variant: 'destructive', title: 'Enter an amount', description: 'Enter the amount you want to fund.' }); return }
    setSubmitting(true)
    try {
      const response = await fetch('/api/payments/paystack/initialize', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ amount: value }) })
      const payload = await response.json()
      if (!response.ok || !payload.authorization_url) throw new Error(payload.error || 'Fiat payment rail is unavailable.')
      window.location.assign(payload.authorization_url)
    } catch (e: any) { toast({ variant: 'destructive', title: 'Fiat funding unavailable', description: e?.message || 'Configure the production payment rail.' }) } finally { setSubmitting(false) }
  }
  ...
  const submitFiat = async () => {
    ...
    setSubmitting(true)
    try {
      const result=await submitDepositRequest({userId:user.id,bankName:session.bankName,accountNumber:session.accountNumber,accountName:session.accountName,amount:fiatAmount,senderBank,reference,expiryTime:session.expiryTime})
    ...
    <div className="mx-auto max-w-[520px] space-y-4 px-4 py-5">
      {stage === 'choose' && <>
        ...
        {method === 'fiat' && <Card className="rounded-[28px] border-[#E1D5D1] bg-[#FFFDFB] p-5 shadow-none">
          ...
          <button disabled={submitting} onClick={startFiatCheckout} className="mt-4 flex h-12 w-full items-center justify-center gap-2 rounded-2xl bg-[#D86F68] text-[11px] font-black text-white disabled:opacity-50">{submitting ? <Loader2 className="animate-spin" size={17}/> : <>Pay securely with Paystack <ArrowRight size={15}/></>}</button>
          <button disabled={submitting} onClick={startStripeCheckout} className="mt-2 flex h-12 w-full items-center justify-center gap-2 rounded-2xl border border-[#8F7650] bg-[#2A2520] text-[11px] font-black text-[#F3DFC0] disabled:opacity-50">{submitting ? <Loader2 className="animate-spin" size={17}/> : <>Pay securely with Stripe <ArrowRight size={15}/></>}</button>
