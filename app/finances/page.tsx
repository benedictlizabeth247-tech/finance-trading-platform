...
].map(([firm, name, category, domain, holdings]) => ({ name, firm, category, logo: `https://www.google.com/s2/favicons?domain=${domain}&sz=128`, source: category === "Public filing" ? `https://${domain}` : `https://${domain}`, holdings }))
...
export default function FinancesScreen() {
  ...
  const quoteBySymbol = useMemo(() => Object.fromEntries(Object.values(quotes).map((quote) => [quote.symbol.toUpperCase(), quote])), [quotes])
  ...
  const quote = selectedSymbol ? quoteBySymbol[selectedSymbol] : null
  ...
  const openMarketOverview = async (symbol: string) => {
    ...
    try {
      const response = await fetch(`/api/market/candles?id=stock.${encodeURIComponent(symbol)}&timeframe=1D`, { cache: "no-store" })
