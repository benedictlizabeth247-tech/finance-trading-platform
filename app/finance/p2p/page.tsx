  ...
  const handleInitiateTrade = async (fiat: number, asset: number) => {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        buyer_id: user.id,
        seller_id: selectedAd.advertiser_id,
        trade_amount_fiat: fiat,
        trade_amount_asset: asset,
        status: 'pending_payment',
        seller_bank_details: {
          bank_name: 'Moniepoint MFB',
          account_number: '5051528892',
          account_name: selectedAd.advertiser_name
        }
      })
      .select()
      .single()
