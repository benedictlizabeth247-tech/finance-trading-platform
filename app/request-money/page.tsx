  ...
  return (
    <main className="min-h-screen bg-background pb-28">
      ...
      <section className="space-y-5 px-6">
        ...
        <label className="block text-sm font-bold text-foreground">Amount<input inputMode="decimal" value={amount} onChange={e => setAmount(e.target.value)} placeholder="₦0.00" className="mt-2 w-full rounded-2xl border border-gray-100 bg-white p-4 text-sm outline-none" /></label>
        ...
        <button disabled={!recipient.trim() || !Number(amount)} onClick={() => setSent(true)} className="w-full rounded-2xl bg-primary py-4 font-bold text-white disabled:opacity-40">Send request</button>
      </section>
      <BottomNav />
    </main>
  )
