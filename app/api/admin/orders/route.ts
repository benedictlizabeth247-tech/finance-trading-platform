...
export async function PATCH(request: Request) {
  ...
  const { data, error } = await client.rpc('admin_settle_order', { p_order_id: body?.id, p_status: status, p_filled_quantity: body?.filled_quantity ?? null, p_admin_note: String(body?.admin_note ?? '') })
