...
export async function getTradingAccount() {
  ...
  if (!userId) return null as TradingAccount | null
  const { data: wallet, error } = await supabase.from('wallets').select('user_id, available, updated_at').eq('user_id', userId).maybeSingle()
  if (error || !wallet) return null as TradingAccount | null
  const available = Number(wallet.available ?? 0)
  return { user_id: userId, funding_balance: available, spot_balance: available, futures_balance: available, updated_at: wallet.updated_at ?? new Date().toISOString() } as TradingAccount
}
...
export async function placeInternalOrder(input: {
  ...
  const response = await fetch('/api/trading/execute', {
    ...
    body: JSON.stringify({ ...input, orderType: input.orderType, leverage: input.leverage ?? 1 }),
  })
