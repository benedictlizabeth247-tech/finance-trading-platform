...
export async function getCandles(
  ...
  // Only indices have a suitable spot-tracking fund; commodity futures do not,
  // so their charts stay with Yahoo / the keyed providers.
  if (type !== 'index') return []
  const proxy = CHART_PROXY[symbol.toUpperCase()]
  if (!proxy) return []
  return nasdaq.getCandles('etf', proxy, timeframe)
}

/** The fund used to chart a given index, surfaced so the UI can disclose it. */
export const chartProxyFor = (symbol: string): string | undefined =>
  CHART_PROXY[symbol.toUpperCase()]
