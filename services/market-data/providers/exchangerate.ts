...
/* -------------------------------------------------------------------------- */
/* History (ECB daily reference rates)                                        */
/* -------------------------------------------------------------------------- */

/**
 * The `open.er-api.com` endpoint exposes latest rates only, so chart history
 * comes from the ECB reference series published by Frankfurter — also keyless.
 * Without it, forex charts render empty on any host Yahoo rate-limits, since
 * every other history source in the forex chain needs an API key.
 *
 * These are daily closes, so intraday timeframes are served as daily bars; the
 * ECB publishes no sub-daily data. The 30 ECB currencies do not include NGN, so
 * the Naira pairs still require Yahoo or a keyed provider.
 */
...
export async function getCandles(
  ...
  const path = `/${isoDate(start)}..${isoDate(end)}?base=${parts.base}&symbols=${parts.quote}`

  const json = await cached(`${PROVIDER}:history:${parts.base}${parts.quote}:${days}`, TTL, () =>
    fetchJson<TimeseriesResponse>(PROVIDER, `${HISTORY_BASE}${path}`, { retries: 1 }),
  )

  const series = Object.entries(json.rates ?? {})
    .map(([date, rates]) => ({ time: Date.parse(`${date}T00:00:00Z`), value: rates[parts.quote] }))
